...
precedence = (
    ...
    ('left', 'COMMA'),
    ('right', 'EQUALS', 'PLUS_EQUAL', 'MINUS_EQUAL', 'MUL_EQUAL', 'EXP_EQUAL', 'DIV_EQUAL',
    ...
    ('nonassoc', 'IS_SMALLER', 'IS_SMALLER_OR_EQUAL',
     'IS_GREATER', 'IS_GREATER_OR_EQUAL', 'SPACESHIP'),
    ('left', 'SL', 'SR'),
    ...
    ('right', 'BOOLEAN_NOT'),
    ('nonassoc', 'INSTANCEOF'),
    ('left', 'OBJECT_OPERATOR', 'DOUBLE_ARROW', 'DOUBLE_COLON'),
    ('right', 'NOT', 'INT_CAST', 'DOUBLE_CAST', 'STRING_CAST',
...
def p_use_declaration(p):
    ...
    if len(p) == 2:
        p[0] = ast.UseDeclaration(p[1], None, None, lineno=p.lineno(1))
    elif len(p) == 3:
        if p[1] == 'function' or p[1] == 'const':
            p[0] = ast.UseDeclaration(p[2], None, p[1], lineno=p.lineno(1))
        else:
            p[0] = ast.UseDeclaration(
                p[1] + p[2], None, None, lineno=p.lineno(1))
    elif len(p) == 4:
        p[0] = ast.UseDeclaration(p[1], p[3], None, lineno=p.lineno(2))
    elif len(p) == 5:
        if p[1] == 'function':
            p[0] = ast.UseDeclaration(
                p[2], p[4], 'function', lineno=p.lineno(1))
        else:
            p[0] = ast.UseDeclaration(
                p[1] + p[2], p[4], None, lineno=p.lineno(1))
...
def p_inner_statement(p):
    '''inner_statement : statement
                       ...
                       | HALT_COMPILER LPAREN RPAREN SEMI'''
    assert len(
        p) == 2, "__HALT_COMPILER() can only be used from the outermost scope"
    p[0] = p[1]


def p_inner_statement_yield_from(p):
    '''inner_statement : YIELD FROM expr_all SEMI'''
    p[0] = ast.YieldFrom(p[3], lineno=p.lineno(1))


def p_inner_statement_yield(p):
    '''inner_statement : YIELD SEMI
                       | YIELD expr_all SEMI
                       | YIELD expr_all DOUBLE_ARROW expr SEMI'''
    if len(p) == 3:
...
def p_statement_if(p):
    '''statement : IF LPAREN expr_all RPAREN statement elseif_list else_single
                 | IF LPAREN expr_all RPAREN COLON inner_statement_list new_elseif_list new_else_single ENDIF SEMI'''
    if len(p) == 8:
...
def p_statement_while(p):
    'statement : WHILE LPAREN expr_all RPAREN while_statement'
    p[0] = ast.While(p[3], p[5], lineno=p.lineno(1))


def p_statement_do_while(p):
    'statement : DO statement WHILE LPAREN expr_all RPAREN SEMI'
...
def p_statement_foreach(p):
    'statement : FOREACH LPAREN expr_all AS foreach_variable foreach_optional_arg RPAREN foreach_statement'
    if p[6] is None:
...
def p_statement_switch(p):
    'statement : SWITCH LPAREN expr_all RPAREN switch_case_list'
...
def p_additional_catches(p):
    '''additional_catches : additional_catches CATCH LPAREN catches_fully_qualified_class_name_list VARIABLE RPAREN LBRACE inner_statement_list RBRACE
                          | empty'''
    if len(p) == 10:
        p[0] = p[1] + [ast.Catch(p[4], ast.Variable(p[5], lineno=p.lineno(5)),
                                 p[8], lineno=p.lineno(2))]
    elif len(p) == 9:
        p[0] = [ast.Catch(p[3], ast.Variable(
            p[4], lineno=p.lineno(5)), p[7], lineno=p.lineno(2))]
    else:
        p[0] = []


def p_catches_class_list(p):
    '''catches_fully_qualified_class_name_list : catches_fully_qualified_class_name_list OR class_name
                                               | class_name'''
    if len(p) == 2:
        p[0] = [p[1]]
    else:
...
def p_elseif_list(p):
    '''elseif_list : empty
                   | elseif_list ELSEIF LPAREN expr_all RPAREN statement'''
    if len(p) == 2:
        p[0] = []
    else:
...
def p_new_elseif_list(p):
    '''new_elseif_list : empty
                       | new_elseif_list ELSEIF LPAREN expr_all RPAREN COLON inner_statement_list'''
    if len(p) == 2:
        p[0] = []
    else:
...
def p_non_empty_for_expr(p):
    '''non_empty_for_expr : non_empty_for_expr COMMA expr_all
                          | expr_all'''
    if len(p) == 4:
        p[0] = p[1] + [p[3]]
    else:
...
def p_foreach_variable(p):
    ...
    elif len(p) == 4:  # LBRACKET array_pair_list RBRACKET
...
def p_case_list(p):
    '''case_list : empty
                 | case_list CASE expr_all case_separator inner_statement_list
...
def p_function_declaration_statement(p):
    '''function_declaration_statement : FUNCTION is_reference STRING LPAREN parameter_list RPAREN returntype LBRACE inner_statement_list RBRACE'''
    ...
    if len(p) == 8:
        ...
        for s in p[6]:
            if isinstance(s, ast.TraitUse):
                ...
                stmts.append(s)
        p[0] = ast.Class(p[2], p[1], p[3], p[4], traits,
                         stmts, lineno=p.lineno(2))
    elif len(p) == 7:
...
def p_fully_qualified_class_name_list(p):
    '''fully_qualified_class_name_list : fully_qualified_class_name_list COMMA class_name 
                                       | class_name'''
    if len(p) == 4:
        p[0] = p[1] + [p[3]]
    else:
...
def p_trait_member(p):
    '''trait_member : STATIC DOUBLE_COLON STRING
                    | class_name DOUBLE_COLON STRING
                    | STRING'''
    if len(p) == 4:
...
def p_trait_modifier(p):
    ...
    p[0] = ast.TraitModifier(p[1], p[3], None,
                             None, lineno=p.lineno(2))


def p_trait_insteadof_modifier(p):
    '''trait_modifier : trait_member INSTEADOF STRING SEMI'''
...
def p_trait_modifier_with_visibility(p):
    ...
    if len(p) == 6:
        p[0] = ast.TraitModifier(p[1], p[4], None, p[3])
    else:
        p[0] = ast.TraitModifier(p[1], None, None, p[3])
...
def p_trait_statement(p):
    '''trait_statement : method_modifiers FUNCTION is_reference function_name LPAREN parameter_list RPAREN returntype method_body
                       | variable_modifiers class_variable_declaration SEMI
                       | USE class_name LBRACE trait_modifiers_list RBRACE
                       | USE fully_qualified_class_name_list SEMI'''
    if len(p) == 10:
        p[0] = ast.Method(p[4], p[1], p[6], p[9], p[3],
                          p[8], lineno=p.lineno(2))
    elif len(p) == 6:
...
def p_class_statement(p):
    '''class_statement : method_modifiers FUNCTION is_reference function_name LPAREN parameter_list RPAREN returntype method_body
                       | variable_modifiers class_variable_declaration SEMI
                       | class_constant_declaration SEMI
                       | USE class_name LBRACE trait_modifiers_list RBRACE
                       | USE fully_qualified_class_name_list LBRACE trait_modifiers_list RBRACE
                       | USE fully_qualified_class_name_list SEMI'''
    if len(p) == 10:
        p[0] = ast.Method(p[4], p[1], p[6], p[9], p[3],
                          p[8], lineno=p.lineno(2))
...
def p_class_variable_declaration_initial(p):
    '''class_variable_declaration : class_variable_declaration COMMA VARIABLE EQUALS static_scalar
                                  ...
                                  | STRING VARIABLE EQUALS static_scalar
                                  | ARRAY VARIABLE EQUALS static_scalar
                                  | QUESTION STRING VARIABLE EQUALS static_scalar
                                  | QUESTION ARRAY VARIABLE EQUALS static_scalar'''

    if len(p) == 6 and p[1] == '?':
        p[0] = [ast.ClassVariable(p[3], p[5], lineno=p.lineno(1))]
    elif len(p) == 6:
        p[0] = p[1] + [ast.ClassVariable(p[3], p[5], lineno=p.lineno(2))]
    elif len(p) == 4:
        p[0] = [ast.ClassVariable(p[1], p[3], lineno=p.lineno(1))]
    elif len(p) == 5:  # STRING VARIABLE EQUALS static_scalar
        p[0] = [ast.ClassVariable(p[2], p[4], lineno=p.lineno(1))]
...
def p_interface_list(p):
    '''interface_list : interface_list COMMA class_name
                      | class_name'''
    if len(p) == 4:
        p[0] = p[1] + [p[3]]
    else:
...
def p_returntype(p):
    '''returntype : COLON ARRAY
                  | COLON class_name
                  | COLON STRING
                  | COLON QUESTION ARRAY
                  | COLON QUESTION STRING
                  | COLON QUESTION class_name
                  | empty'''
    if len(p) == 3:
        p[0] = p[2]
    elif len(p) == 4:  # 这里忽略了?，如果需要准确解析，则需要给参数增加相应属性
        p[0] = p[3]
    else:
        p[0] = None
...
def p_parameter_list(p):
    '''parameter_list : parameter_list COMMA parameter
                      ...
                      | parameter'''
    if len(p) == 4:  # parameter_list COMMA parameter的情况
        p[0] = p[1] + [p[3]]
    elif len(p) == 3:  # parameter_list COMMA的情况
        p[0] = [p[1]]
    else:
        p[0] = [p[1]]
...
# ? int & ... $test = xxx
# is_nullable parameter_type is_reference is_splat

def p_parameter(p):
    '''parameter : is_nullable parameter_type is_reference is_splat VARIABLE
                 | is_nullable parameter_type is_reference is_splat VARIABLE EQUALS static_scalar'''

    if len(p) == 6:
        p[0] = ast.FormalParameter(p[5], None, p[3], p[2], lineno=p.lineno(1))
    elif len(p) == 8:
        p[0] = ast.FormalParameter(p[5], p[7], p[3], p[2], lineno=p.lineno(1))
...
def p_assignment(p):
    '''assignment : expr EQUALS expr_all
                  | expr EQUALS AND expr_all
                  | LBRACKET array_pair_list RBRACKET EQUALS expr_all'''
    ...
    if len(p) == 4:
        ...
        p[0] = ast.Assignment(p[2], p[5], False, lineno=p.lineno(2))
...
def p_expr_new(p):
    '''expr : NEW CLASS extends_from implements_list LBRACE class_statement_list RBRACE
            | NEW class_name_reference ctor_arguments
            | NEW STATIC ctor_arguments
            | NEW CLASS LPAREN parameter_list RPAREN extends_from implements_list LBRACE class_statement_list RBRACE
            | NEW CLASS LPAREN function_call_parameter_list RPAREN extends_from implements_list LBRACE class_statement_list RBRACE'''

    if len(p) == 8:
        p[0] = ast.New(p[6], None, lineno=p.lineno(1))
    elif len(p) == 4:
        p[0] = ast.New(p[2], p[3], lineno=p.lineno(1))
    elif len(p) == 11:
        p[0] = ast.New(p[9], None, lineno=p.lineno(1))
...
def p_expr_objectop(p):
    ...
    if params is not None:
        ...
        p[0] = ast.ArrayOffset(
            ast.ObjectProperty(p[1], name, lineno=p.lineno(2)),
            _dims[0][1],
            lineno=_dims[0][2]
        )
...
def p_expr_list_assign(p):
    'assignment : LIST LPAREN assignment_list RPAREN EQUALS expr_all'
...
def p_function_call(p):
    '''function_call : namespace_name LPAREN function_call_parameter_list RPAREN
                     | function_call LPAREN function_call_parameter_list RPAREN
                     | variable LPAREN function_call_parameter_list RPAREN
    ...
    if len(p) == 5:
        ...
        if p[1] == '(':
            p[0] = ast.FunctionCall(p[2], p[5], lineno=p.lineno(2))
        else:
            p[0] = ast.FunctionCall(
                p[1] + p[2] + p[3], p[5], lineno=p.lineno(1))


# def p_function_call_static_classname(p):
#     '''function_call_static_classname : STATIC
#                                       | class_name
#                                       | reference_variable
#                                       | expr'''
...
# reference_variable variable_class_name

def p_function_call_static(p):
    '''function_call : STATIC DOUBLE_COLON function_name LPAREN function_call_parameter_list RPAREN
                     | class_name DOUBLE_COLON function_name LPAREN function_call_parameter_list RPAREN
                     | reference_variable DOUBLE_COLON function_name LPAREN function_call_parameter_list RPAREN
                     | expr DOUBLE_COLON function_name LPAREN function_call_parameter_list RPAREN
                     | expr DOUBLE_COLON VARIABLE LPAREN function_call_parameter_list RPAREN'''

    # '''function_call : function_call_static_classname DOUBLE_COLON function_name LPAREN function_call_parameter_list RPAREN'''
...
def p_function_call_static_dynamic_name(p):
    '''function_call : class_name DOUBLE_COLON LBRACE expr RBRACE LPAREN function_call_parameter_list RPAREN
                     | reference_variable DOUBLE_COLON LBRACE expr RBRACE LPAREN function_call_parameter_list RPAREN'''
...
def p_static_member(p):
    '''static_member : STATIC DOUBLE_COLON variable_without_objects
                     | class_name DOUBLE_COLON variable_without_objects
                     | reference_variable DOUBLE_COLON variable_without_objects
                     | class_name DOUBLE_COLON LBRACE expr RBRACE
                     | reference_variable DOUBLE_COLON LBRACE expr RBRACE'''
    if len(p) == 4:
        p[0] = ast.StaticProperty(p[1], p[3], lineno=p.lineno(2))
    else:
...
def p_variable_array_offset(p):
    '''variable : variable LBRACKET dim_offset RBRACKET
                | class_constant LBRACKET dim_offset RBRACKET
...
#     'object_dim_list : object_dim_list LBRACE expr_all RBRACE'
#     p[0] = p[1] + [(ast.StringOffset, p[3], p.lineno(2))]


def p_variable_name(p):
    '''variable_name : STRING
                     | LBRACE expr_all RBRACE'''
    if len(p) == 2:
        p[0] = p[1]
    else:
        p[0] = p[2]
...
def p_non_empty_array_pair_list_item(p):
    '''non_empty_array_pair_list : non_empty_array_pair_list COMMA AND variable
                                 | non_empty_array_pair_list COMMA expr_all
                                 | non_empty_array_pair_list COMMA empty
                                 | AND variable
                                 | empty COMMA expr_all
                                 | expr_all'''
    if len(p) == 5:
        p[0] = p[1] + [ast.ArrayElement(None, p[4], True, lineno=p.lineno(2))]
    elif len(p) == 4 and p[1] == None:
        p[0] = [] + [ast.ArrayElement(None, p[3], False, lineno=p.lineno(2))]
    elif len(p) == 4:
        p[0] = p[1] + [ast.ArrayElement(None, p[3], False, lineno=p.lineno(2))]
    elif len(p) == 3 and p[1] == '&':
        p[0] = [ast.ArrayElement(None, p[2], True, lineno=p.lineno(1))]
    elif len(p) == 3 and p[1] == ',':
        p[0] = [ast.ArrayElement(None, None, None, lineno=p.lineno(1))]
    else:
...
def p_function_call_parameter_list(p):
    '''function_call_parameter_list : function_call_parameter_list COMMA function_call_parameter
                                    | function_call_parameter_list COMMA
...
def p_function_call_parameter(p):
    '''function_call_parameter : expr_all
                               | AND variable
                               | SPLAT variable'''
    if len(p) == 2:
...
def p_expr_function(p):
    '''expr : is_static FUNCTION is_reference LPAREN parameter_list RPAREN lexical_vars returntype LBRACE inner_statement_list RBRACE
            | is_static FN is_reference LPAREN parameter_list RPAREN returntype DOUBLE_ARROW expr'''

    if p[2] == 'fn':
        p[0] = ast.Closure(p[5], None, [p[9]], p[3], lineno=p.lineno(1))

    elif p[2] == 'function':
        p[0] = ast.Closure(p[5], p[7], p[10], p[3], lineno=p.lineno(1))
...
def p_assignment_op(p):
    ...
    #         | variable DOUBLE_QUESTION_EQUAL expr'''
    '''expr : expr PLUS_EQUAL expr
            | expr MINUS_EQUAL expr
            | expr MUL_EQUAL expr
            | expr EXP_EQUAL expr
            | expr DIV_EQUAL expr
            | expr CONCAT_EQUAL expr
            | expr MOD_EQUAL expr
            | expr AND_EQUAL expr
            | expr OR_EQUAL expr
            | expr XOR_EQUAL expr
            | expr SL_EQUAL expr
            | expr SR_EQUAL expr
            | expr DOUBLE_QUESTION_EQUAL expr'''
...
def p_expr_binary_op(p):
    '''expr : expr BOOLEAN_AND expr
            ...
            | expr_all LOGICAL_AND expr_all
            | expr_all LOGICAL_OR expr_all
            | expr_all LOGICAL_XOR expr_all
            | expr AND expr
            | expr OR expr
            | expr XOR expr
            | expr CONCAT expr
...
def p_expr_empty(p):
    'expr : EMPTY LPAREN expr_all RPAREN'
...
def p_class_constant(p):
    '''class_constant : STATIC DOUBLE_COLON STRING
                      | class_name DOUBLE_COLON STRING
                      | reference_variable DOUBLE_COLON STRING
                      | expr DOUBLE_COLON STRING
                      | expr DOUBLE_COLON VARIABLE'''
    p[0] = ast.StaticProperty(p[1], p[3], lineno=p.lineno(2))
...
def p_encaps_var_curly_variable(p):
    '''encaps_var : CURLY_OPEN expr RBRACE'''
    p[0] = p[2]
...
def p_error(t):
    if t:
        raise SyntaxError('invalid syntax'+'; Token: '+str(t),
                          (None, t.lineno, None, t.value))
    else:
